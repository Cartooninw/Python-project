    ...
    while BreakLoopanywhereyouwant:# While ทณู
        while True:
            try:
                ...
                Login_input = input("\nLogin Page\nEnter your (leave space and Enter for register)(username/password or pin):")
                if Login_input in "                                 ":
                    Login =  "Failid"
                    break
                elif Login_input == "leave":
                    Login = None
                    break
                else:
        ...
        while attemps != 0:
            if Login == "Pass":
                account_balance = data.loc[data["ID"] == split_login[0] ,"Balance"].item()
                Cash = data.loc[data["ID"] == split_login[0] ,"Cash"].item()
                while True:
                    while True:
                        try:
                            user.CheckBalance(User_system.cur_path + "/atmdata.xlsx")
                    ...
                    if withdraw == "Successful":
                        ...
                        if again == "y":
                            data.loc[data["ID"] == split_login[0] ,"Balance"] = account_balance
                            data.loc[data["ID"] == split_login[0] ,"Cash"] = Cash
                        ...
                        if withdraw == "out of scale of banknote":
                            ...
                            while ChangeTrue:
                                ...
                                if passpinask.lower() == "pass":
                                    ...
                                    data.loc[data["ID"] == split_login[0],"Password"] = newpass
                                    ...
                                    while True:
                                        try:
                                            int(newpin)
                                            if len(str(newpin)) == 4:
                                                data.loc[data["ID"] == split_login[0],"Pin"] = newpin
